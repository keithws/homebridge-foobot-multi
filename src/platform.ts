 ...
 */
export class FoobotHomebridgePlatform implements DynamicPlatformPlugin {
  ...
  async discoverDevices() {
    ...
    try {
      ...
      const delay = Math.round(10 * Math.random() * 60 * 1000);
      setTimeout(this.discoverDevices.bind(this), delay);
    ...
    for (const device of devices) {
      ...
      // add `offset` property to each device
      device.offset = devices.indexOf(device);
      ...
      if (existingAccessory) {
        ...
        // this is imported from `platformAccessory.ts`
        new FoobotPlatformAccessory(this, accessory);
